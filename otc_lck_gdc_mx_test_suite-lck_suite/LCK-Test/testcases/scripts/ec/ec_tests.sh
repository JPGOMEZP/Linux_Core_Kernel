...
if [ $KERNEL_TYPE == "builtin" ]; then
  run_test "ec-func-builtin-tests"
elif [ $KERNEL_TYPE == "module" ]; then
  case $TEST_TYPE in
    func)   run_test "ec-func-tests" ;;
    perf)   run_test "ec-perf-tests" ;;
    stress) run_test "ec-stress-tests" ;;
    bat)    run_test "ec-bat-tests" ;;
    full)   run_test "ec-full-tests" ;;
  esac
fi
