...
if [ $KERNEL_TYPE == "builtin" ]; then
  run_test "lpc-func-builtin-tests"
elif [ $KERNEL_TYPE == "module" ]; then
  case $TEST_TYPE in
    func)   run_test "lpc-func-tests" ;;
    perf)   run_test "lpc-perf-tests" ;;
    stress) run_test "lpc-stress-tests" ;;
    bat)    run_test "lpc-bat-tests" ;;
    full)   run_test "lpc-full-tests" ;;
  esac
fi
