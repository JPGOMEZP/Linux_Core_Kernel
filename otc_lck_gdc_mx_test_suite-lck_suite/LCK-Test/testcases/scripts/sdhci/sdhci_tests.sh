...
if [ $KERNEL_TYPE == "builtin" ]; then
  run_test "sdhci-func-builtin-tests"
elif [ $KERNEL_TYPE == "module" ]; then
  case $TEST_TYPE in
    func)   run_test "sdhci-func-tests" ;;
    perf)   run_test "sdhci-perf-tests" ;;
    stress) run_test "sdhci-stress-tests" ;;
    bat)    run_test "sdhci-bat-tests" ;;
    full)   run_test "sdhci-full-tests" ;;
  esac
fi
