...
#
# Aug, 2016.
#   Juan Pablo Gomez Preciado <juan.p.gomez@intel.com>
#	-Initial draft
#	-get_platform function added
#
##############################################################################
source "common.sh"
...
test_print_trc "PLATFORM: $PRODUCT_NAME"
#********************************************************
...
case $Platform in
bxt)
    GPIO_NUMS="365,366,367"
    GPIO_NUM_SKIPPED="293"
#***********************************************************
      ...
      while [ $i -lt $TEST_LOOP ]; do
        ...
        if [ -e $GPIO_SYSFS_DIR/gpio"$GPIO_NUM" ]; then
          ...
          pm_context_restore)
            ...
            ;;
			drv_check)
				do_cmd gpio_sysentry_drv_check || die "gpio_sysentry_drv_check failed"
			;;
			acpi_enum)
				do_cmd gpio_sysentry_acpi_enum || die "gpio_sysentry_acpi_enum failed"
			;;
          esac
        else
