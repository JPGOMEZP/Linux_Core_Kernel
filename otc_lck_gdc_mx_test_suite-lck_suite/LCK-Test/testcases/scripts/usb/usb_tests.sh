...
if [ $KERNEL_TYPE == "builtin" ]; then
  run_test "usb-func-builtin-tests"
elif [ $KERNEL_TYPE == "module" ]; then
  case $TEST_TYPE in
    func)   run_test "usb-func-tests" ;;
    perf)   run_test "usb-perf-tests" ;;
    stress) run_test "usb-stress-tests" ;;
    bat)    run_test "usb-bat-tests" ;;
    full)   run_test "usb-full-tests" ;;
  esac
fi
