...
get_xml_childs() {
  ...
  while read -r line; do
    ...
    if [[ $line =~ \<${root}[\>|\s.*\>] ]] && [ $in -eq 0 ]
    then
      in=1
      count=$((count + 1))
      continue
    elif [[ $line =~ \</${root}\> ]] && [ $in -eq 1 ]
    then
      break
    fi
    ...
    if [ $in -eq 1 ]; then
      ...
      if [ $in_child -eq 0 ] && [[ $line =~ \<[^!].*\> ]]
      then
        in_child=$((in_child + 1))
        child=$(echo $line | grep -Eo '\<[a-zA-Z0-9_\.]*' | head -n 1 |tr '<|>|\n' ' ' | xargs)
        childs="$childs $child:$count"
      elif [ $in_child -gt 0 ] && [[ $line =~ \<${child}\> ]]
      then
        in_child=$((in_child + 1))  # We found a subchild with the child's tag
      fi

      # Close first level childs
      if [ $in_child -gt 0 ] && [[ $line =~ \</${child}\> ]]
      then
        in_child=$((in_child - 1))
      fi
    fi
...
get_xml_attributes() {
  ...
  local line_txt=$(sed -n "${line_num}p" < $XML_FILE)
