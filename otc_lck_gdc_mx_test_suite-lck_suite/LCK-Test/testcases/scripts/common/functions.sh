...
frequnit() {
    ...
    local ghz=$(echo "$freq 1000000" |awk '{printf "%.1f", $1/$2}')
    local mhz=$(echo "$freq 1000" |awk '{printf "%.1f", $1/$2}')
...
suspend()
{
    ...
    while [ $i -lt $iterations ]; do
      ...
      if [ -e $RTC0_PATH/$ALARM ]; then
          ...
          echo `expr $(cat $RTC0_PATH/since_epoch) + "${suspend_time}"` > $RTC0_PATH/$ALARM # set alarm
...
check_suspend()
{
    ...
    dmesg | sed "1,/$last_time/d" | grep -i "$expect" && report "suspend successfully" || die "suspend failed"
}
...
check_suspend_fail()
{
    ...
    dmesg | sed "1,/$last_time/d" | grep -i "$expect" && report "suspend failed as expected" || die "suspend did not fail as expected"
}
...
check_resume()
{
    ...
    dmesg | sed "1,/$last_time/d" | grep -i "$expect" && report "resume successfully" || die "resume failed"
}
