...
if [ $KERNEL_TYPE == "builtin" ]; then
  run_test "spi-func-builtin-tests"
elif [ $KERNEL_TYPE == "module" ]; then
  case $TEST_TYPE in
    func)   run_test "spi-func-tests" ;;
    perf)   run_test "spi-perf-tests" ;;
    stress) run_test "spi-stress-tests" ;;
    bat)    run_test "spi-bat-tests" ;;
    full)   run_test "spi-full-tests" ;;
  esac
fi
