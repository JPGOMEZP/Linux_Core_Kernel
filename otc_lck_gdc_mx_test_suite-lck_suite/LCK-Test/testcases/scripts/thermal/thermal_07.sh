...
check_type() {
    ...
    local trip_types=$(ls $path | grep "trip_point_[0-9]_type")
    local tmp=""
    ...
    for ttype in $trip_types; do
        ...
        check "Valid trip type value of $ttype" valid_trip_type $tmp $ttype  || return 1
    done
    return 0
}
...
check_temp() {
    local path=$THERMAL_PATH/$1
    local trip_temps=$(ls $path | grep "trip_point_[0-9]_temp")
    local tmp=0

    test_print_trc "Checking trip point temperatures of $path"
    for temp in $trip_temps; do
        tmp=$(cat $path/$temp)
        check "Temperature value of $temp: $tmp" "test $tmp -ge 0" || return 1
done
    return 0
}
