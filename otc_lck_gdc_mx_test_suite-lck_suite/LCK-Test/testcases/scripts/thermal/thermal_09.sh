...
############################# Functions #######################################
check_power_clamp() {
	local power_clamps=$(grep -Elr 'intel_powerclamp' 2>/dev/null --include=type $THERMAL_PATH | grep -oE 'cooling_device[0-9]+')

	check "Intel Power Clamp exists..." "test " "${#power_clamps[@]} -ne 0" || return 1
