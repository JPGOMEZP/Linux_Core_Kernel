...
check_constraints() {
	local path=$1
	shift 1
	local indices=$(ls $path | grep -Eo 'constraint_[0-9]+_power_limit_uw' | grep -Eo '[0-9]+')
...
check_zones_recursive () {
	...
	local level=$2
	shift 2
	local childs=$(ls $root | grep -E $RGX_PACKAGE)
...
koption=`check_koption "$REQUEST_KCONFIG"` || die "Failed to verify kernel config"
if [ "$koption" == "m" ];then
