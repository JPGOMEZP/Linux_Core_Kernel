...
for_each_thermal_zone() {
    ...
    local zones=$(ls $THERMAL_PATH | grep "thermal_zone['$MAX_ZONE']")
    if [ -z "$zones" ]; then
        log_end "fail";
        return 1;
    fi
...
validate_trip_level() {
    ...
    local tmp=$(cat $dirpath/$trip_type)
    if [ "$tmp" != "passive" ]; then
        check_valid_temp $trip_temp $zone_name 1 || return 1 # Greater or equal 0
    fi
}
...
echo " $zone: $ztemp"
        ...
        echo "ALL ZONE: $ALL_ZONE"
	fi
echo "Estas aqui ALL ZONE: $ALL_ZONE y esto: $zone"
    done
}
...
get_active_tzones_sofia() {
  ...
  local tmp_arr=""
  local my_subchilds=""
  local my_childs=$(get_xml_childs "thermalconfig")
...
get_active_tzones_baytrail() {
  ...
  echo $(cat ${THERMAL_PATH}/thermal_zone*/type)
}
...
############################ USER-DEFINED Params ###############################
#case $SOC in
#  baytrail) ACT_TZONES=$(get_active_tzones_baytrail);;
#  sofia*) load_xml  "$ITUX_XML"
#          ACT_TZONES=$(get_active_tzones_sofia)
#          unload_xml
#          ;;
#  baytrail-t|broadwell-u|skylake-y|skylake-s|kabylake-y|broxton-p) ACT_TZONES=$(get_active_tzones_baytrail);;
