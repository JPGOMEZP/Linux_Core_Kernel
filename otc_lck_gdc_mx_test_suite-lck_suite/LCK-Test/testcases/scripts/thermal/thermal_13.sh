...
check_max_state() {
    ...
    local max_state=$(cat $cdevpath/max_state)
    local cur_state=$(cat $cdevpath/cur_state)
    local prev_state=0
    local invalid_state=$((max_state + 1))

	local is_intel_powerclamp=`cat $cdevpath/type`
	if [ "x$is_intel_powerclamp" != "xintel_powerclamp" ];then
		check "$cdev_name cur_state=$cur_state, max_state=$max_state" "test $max_state -ge $cur_state" || return 1
		...
		check "$cdev_name cur_state=$cur_state, max_state=$max_state" "test $max_state -ge $cur_state"
		if [ $? -ne 0 ]; then
